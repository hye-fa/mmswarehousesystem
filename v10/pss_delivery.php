...
// pss_delivery.php
// PSS MANAGEMENT HUB
// UPDATED: Removed duplicate Qty Box, kept layout compact
...
// Fetch ALL schools for this CO + JOIN HD Name
$stmt = $pdo->prepare("SELECT s.id, s.school_name, s.school_code, s.student_count, s.default_hd_id, s.co_number, s.sap_no, s.tender_no, s.contract_no,
                              h.name as hd_name 
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Selecting an HD will filter the School list below and show their assigned schools on the right.</small>
                        </div>
                        ...
                        <!-- ROW 2: Students, SAP, Bil TP, Packs -->
                        <div class="row g-3 mb-3">
                            <div class="col-md-2">
                             ...
                             <div class="col-md-12">
                                ...
                                <select name="inventory_batch_id" class="form-select border-success fw-bold" required>
                                    <?php foreach($batches as $b): ?>
                                        <option value="<?= $b['id'] ?>">
                                            <?= $b['batch_no'] ?> (Exp: <?= $b['expiry_date'] ?>) - Stock: <?= $b['qty_on_hand'] ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
...
<script>
    ...
    function filterSchoolsByHD() {
        ...
        updateSidebarTable(selectedHD);
    }

    function updateSidebarTable(selectedHD) {
        ...
        schoolsData.forEach(function(school) {
            // Check if this school belongs to selected HD
            // school.default_hd_id might be string or int, so use ==
            if (school.default_hd_id == selectedHD) {
                count++;
                let row = `
                    <tr>
