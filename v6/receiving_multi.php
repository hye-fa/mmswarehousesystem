...
// UPDATED: Category Smart Lock (Prevents mixing PSS/PST/UHT in one DO)
...
<head>
    ...
    <style>
        ...
        .locked-category { background-color: #e9ecef; cursor: not-allowed; border-color: #6c757d; color: #6c757d; }
...
<div class="container-fluid mt-4">
    ...
    <!-- ALERT NOTICES -->
    <div class="alert alert-warning border-warning shadow-sm py-2 mb-3" role="alert">
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <!-- Note: ID main_category used for locking -->
                        <select name="category" class="form-select fw-bold border-primary" id="main_category" onchange="applyCategoryFilter()">
                            <option value="PST" selected>PST (Fresh)</option>
                            <option value="UHT">UHT (Retail)</option>
                            <option value="PSS">PSS (School)</option>
                            <option value="Ice Cream">Ice Cream</option>
                        </select>
                        <small id="cat_lock_msg" class="text-danger" style="display:none; font-size: 0.75rem;">Locked while items are selected.</small>
                    </div>
                </div>
                
                <!-- HYBRID PALLET TALLY TABLE -->
                <div class="row mt-3">
                    <div class="col-md-12">
...
<script>
    ...
    function checkCategoryLock() {
        ...
        const catSelect = document.getElementById('main_category');
        const msg = document.getElementById('cat_lock_msg');

        if(hasSelection) {
            catSelect.disabled = true;
            catSelect.classList.add('locked-category');
            msg.style.display = 'block';
        } else {
            catSelect.disabled = false;
            catSelect.classList.remove('locked-category');
    ...
    function addRow(prefillData = null) {
        ...
        const html = `
            <tr id="${rowId}">
                <td>
                    <select name="items[${globalRowCounter}][product_id]" class="form-select form-select-sm prod-select" required onchange="handleProductChange('${globalRowCounter}')">
                        ${optionsHtml}
                    </select>
                </td>
                <td>
                    <div class="input-group input-group-sm">
                        <input type="text" id="scan_input_${globalRowCounter}" class="form-control qr-input" placeholder="Click to Scan" 
                               onclick="openCameraForRow('${globalRowCounter}')" 
                ...
                <td>
                    <input type="text" name="items[${globalRowCounter}][batch_no]" id="batch_${globalRowCounter}" class="form-control form-control-sm is-filled" readonly required>
                </td>
                <td>
    ...
    // ... (Recalc, CheckVolume, UpdateHeaderTally, Camera, Parser functions same as before) ...
    ...
    function updateHeaderTally() {
        ...
        document.getElementById('total_red').innerText = red + man_red;
        document.getElementById('total_lhp').innerText = greenL + man_lhp;
        document.getElementById('total_orange').innerText = orange + man_orange;
        document.getElementById('total_ffm').innerText = greenF + man_ffm;
        document.getElementById('total_black').innerText = black + man_black;
        document.getElementById('total_plain').innerText = plain + man_plain;

        document.getElementById('grand_auto').value = red + greenL + orange + greenF + black + plain;
        document.getElementById('grand_manual').value = man_red + man_lhp + man_orange + man_ffm + man_black + man_plain;
        document.getElementById('grand_final').innerText = parseInt(document.getElementById('grand_auto').value) + parseInt(document.getElementById('grand_manual').value);
    }
    ...
    function validateMultiForm() {
        // 1. Re-enable Category before Submit
